...
import LogoScatter from "./logo-scatter"
...
export default function QuizQuestion({
  ...
  const [scatterTrigger, setScatterTrigger] = useState<{ x: number; y: number } | null>(null)
  ...
  const handleAnswer = (index: number, buttonElement: HTMLButtonElement) => {
    if (selectedAnswer === null) {
      ...
      if (index === question.correctAnswer) {
        ...
        setShowScatter(true)
        // Hide scatter after animation completes
        setTimeout(() => setShowScatter(false), 1000)
      }
  ...
  return (
    <>
      {scatterTrigger && (
        <LogoScatter isVisible={showScatter} triggerX={scatterTrigger.x} triggerY={scatterTrigger.y} />
      )}
          ...
          <div className="space-y-3 mb-4">
            {question.options.map((option, index) => {
              ...
              return (
                <button
                  key={index}
                  ref={index === question.correctAnswer ? selectedButtonRef : null}
